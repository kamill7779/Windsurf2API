...
import { runChatCore, runToolMockCore, ToolMockResult } from '../../chat.js';
import {
...
export async function handleAnthropicNativeMessage(
  ...
  if (toolResult) {
    ...
    if (!bridgeModelInfo) {
      throw new Error(`Unknown bridge model "${bridge.modelKey}"`);
    }
